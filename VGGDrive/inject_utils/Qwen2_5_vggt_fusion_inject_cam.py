...
class CrossAttentionFusion(nn.Module):
    ...
    def forward(self, F1, F2):
        ...
        Q = self.q_proj(F1).view(B, N, self.num_heads, self.head_dim).transpose(1, 2)  # [B, h, N, d]
        ...
        attn_output = attn_output.transpose(1, 2).contiguous().view(B, N, -1)  # [B, N, 512]
...
class CustomQwen2_5_VLModel(Qwen2_5_VLModel):
    def __init__(self, config):
        ...
        self.prompt_tuning_mlp = nn.ModuleList([
            nn.ModuleList([
                ...
                nn.Sequential(
                    nn.Linear(16, self.in_dim // self.scale),
                    nn.GELU(),
                    nn.Linear(self.in_dim // self.scale, self.in_dim // self.scale),
                ),
    ...
    def forward(
        ...
        img2lidar: Optional[torch.FloatTensor] = None,
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if use_3D_inject:
                ...
                # injnect img2lidar  4*4
                cam_feat = self.prompt_tuning_mlp[idx][2](img2lidar.flatten(2,3))
                vggt_hs = vggt_hs.view(B, S, -1, self.in_dim // self.scale)
                vggt_hs[:, :, 0:1, :] = vggt_hs[:, :, 0:1, :] + cam_feat.unsqueeze(2)
                
                # cross_attention (vision feat & vggt 3D feat)
                enhanced_vision_hs = self.prompt_tuning_mlp[idx][3](vision_hs, vggt_hs.flatten(1,2))

                # Residual
                enhanced_vision_hs = vision_hidden_states + self.prompt_tuning_mlp[idx][4](enhanced_vision_hs)
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None: 
                ...
                if cam2lidar is None:
                    cam2lidar = getattr(self, "_cam2lidar", None)
                img2lidar = torch.tensor(np.array(cam2lidar), dtype=torch.bfloat16, device=pixel_values.device) # B, S, 4, 4
                if img_list is None:
