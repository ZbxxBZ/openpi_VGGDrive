...
class VGGT_dist_loss(torch.nn.Module):
    def __init__(
        self,
        out_tokens_dim=2048,
        in_dim=3584,
        ...
        super(VGGT_dist_loss, self).__init__()
        self.S = S
    ...
    def forward(self, vision_hidden_states, vggt_visual_gt): # [4, 1536, 3584]  [B, S, 782, 2048]
        ...
        B = vggt_visual_gt.shape[0]

        vision_hidden_states = vision_hidden_states.view(B, self.S, -1, self.in_dim).flatten(0,1) # [B*S, 512, 3584]
        vision_hidden_states = self.dim_linear(vision_hidden_states) # # [B*S, 512, 2048]

        if vggt_visual_gt.shape[2] == vision_hidden_states.shape[1]:
            vggt_gt_final = vggt_visual_gt.flatten(0, 1)
        else:
            vggt_visual_gt = vggt_visual_gt.view(B, self.S, -1, self.vggt_w, self.out_tokens_dim) # [B, S, 21, 37, 2048]
            ...
            vggt_gt_final = vggt_gt_pooled.view(B*self.S, self.out_tokens_dim, -1).permute(0, 2, 1) #[B*S, 512, 2048]

        cos_sim = F.cosine_similarity(vision_hidden_states, vggt_gt_final, dim=-1)  # [B*S, 512]
...
class CustomQwen2_5_VLForConditionalGeneration(Qwen2_5_VLForConditionalGeneration):
    def __init__(self, config, **kwargs):
        ...
        self.prompt_tuning_dist = VGGT_dist_loss(
                S=self.num_view,
                H=self.resized_H,
                W=self.resized_W,
            )
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None: # [16384, 1176]   [B*4*32*64, 2*14*14]   
                ...
                image_embeds = image_embeds.to(inputs_embeds.device, inputs_embeds.dtype)# [4096, 3584]
                inputs_embeds = inputs_embeds.masked_scatter(image_mask, image_embeds)  # [2, 2219, 3584]
        ...
        if self.training:
            ...
            for b in range(last_hidden_states.size(0)):
                hidden_b = last_hidden_states[b]
                mask_b = mask[b]
                visual_tokens_b = hidden_b[mask_b]
                vision_hidden_states.append(visual_tokens_b)
            vision_hidden_states = torch.stack(vision_hidden_states, dim=0)  # [4, 1536, 3584]  torch.Size([4, 2331, 3584])

            vggt_visual_gt = run_vggt_inference(
                imgs_tensor.to(inputs_embeds.device),
                device=inputs_embeds.device
            ) # [B, S, 782, 2048] torch.Size([4, 3, 782, 2048])

            dist_align_loss = self.prompt_tuning_dist(vision_hidden_states, vggt_visual_gt) # [4, 1536, 3584]  [B, S, 782, 2048]
            loss = loss + dist_align_loss
