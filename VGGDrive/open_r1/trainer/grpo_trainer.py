...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        messages = [x["message"] for x in inputs]
        prompts = [x["message"][0]["content"][-1]["text"] for x in inputs]
        text_list = [self.processing_class.apply_chat_template(i_message, tokenize=False, add_generation_prompt=True) for i_message in messages]
        image_inputs = sum([x["image_inputs"] for x in inputs if x["image_inputs"] is not None], [])
