...
class ResampleCallback(TrainerCallback):
    def __init__(self, dataset, data_args) -> None:
        super().__init__()
        self.dataset = dataset
        self.ratio_shuffle = data_args.ratio_shuffle
        self.mixed_shuffle = data_args.mixed_shuffle

    def on_epoch_begin(self, args, state, control, **kwargs):
        print('ResampleCallback Epoch', state.epoch)

        if self.ratio_shuffle:
            self.dataset.local_shuffle()
        if self.mixed_shuffle:
            random.shuffle(self.dataset.shuffled_samples)
        self.dataset.print_shuffled_samples()
        return control

CALLBACKS = {
    "push_to_hub_revision": PushToHubRevisionCallback,
    "resample": ResampleCallback
}

def get_callbacks(dataset, data_args, train_config, model_config) -> List[TrainerCallback]:
    callbacks = []
    ...
    if "resample" not in train_config.callbacks:
        callbacks.append(ResampleCallback(dataset, data_args))
    return callbacks
