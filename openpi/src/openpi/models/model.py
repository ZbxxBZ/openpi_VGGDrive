...
class Observation(Generic[ArrayT]):
    ...
    # Optional calibrated camera-to-world (or common robot-frame) transforms for CVGE.
    # Keys match `images`; all cameras must use the same reference frame and units.
    camera_to_world: dict[str, at.Float[ArrayT, "*b 4 4"]] | None = None
    ...
    def from_dict(cls, data: at.PyTree[ArrayT]) -> "Observation[ArrayT]":
        ...
        return cls(
            ...
            camera_to_world=data.get("camera_to_world"),
        )
...
class BaseModelConfig(abc.ABC):
    ...
    def load_pytorch(self, train_config, weight_path: str):
        ...
        model = pi0_pytorch.PI0Pytorch(config=train_config.model, load_geometry_weights=False)
        load_pi0_weights(model, weight_path)
        return model
