...
class PI0Pytorch(nn.Module):
    def __init__(self, config, *, load_geometry_weights: bool = True):
        super().__init__()
        self.config = config
        ...
        if config.geometry.enabled and config.pytorch_compile_mode is not None:
            logging.info("CVGE uses eager sampling to support batches with different valid camera sets")
        elif config.pytorch_compile_mode is not None:
        ...
        if config.geometry.enabled:
            unknown_keys = set(config.geometry.image_keys) - set(_preprocessing.IMAGE_KEYS)
            if unknown_keys:
                raise ValueError(f"Unknown geometry image keys: {unknown_keys}")
            self.vggt_encoder = VGGTEncoder(config.geometry, load_weights=load_geometry_weights)
            self.configure_geometry_training()

    def configure_geometry_training(self):
        """Freeze weights without cutting the gradient path from actions to CVGE."""
        policy = self.config.geometry.train_policy
        self.requires_grad_(policy == "full")
        self.paligemma_with_expert.cvge.requires_grad_(requires_grad=True)
        if policy == "adapter_action":
            self.paligemma_with_expert.gemma_expert.requires_grad_(requires_grad=True)
            for name in (
                "action_in_proj",
                "action_out_proj",
                "state_proj",
                ...
                if hasattr(self, name):
                    getattr(self, name).requires_grad_(requires_grad=True)
        self.vggt_encoder.requires_grad_(requires_grad=False)

    def train(self, mode: bool = True):  # noqa: FBT001, FBT002 -- nn.Module API
        super().train(mode)
        if self.config.geometry.enabled:
            self.vggt_encoder.eval()
            if self.config.geometry.train_policy != "full":
                self.paligemma_with_expert.paligemma.eval()
            if self.config.geometry.train_policy == "adapter_only":
    ...
    def _prepare_geometry(self, geometry_images, image_masks, observation):
        ...
        if self.config.geometry.use_camera_pose:
            pose_dict = getattr(observation, "camera_to_world", None) or {}
            pose_list = []
            for key, mask in zip(self.config.geometry.image_keys, selected_masks, strict=True):
    ...
    def _apply_checkpoint(self, func, *args, **kwargs):
        ...
        if self.gradient_checkpointing_enabled and self.training:
            return torch.utils.checkpoint.checkpoint(
                func, *args, use_reentrant=False, preserve_rng_state=self.config.geometry.enabled, **kwargs
            )
    ...
    def embed_prefix(
        ...
        # Fixed leading image slots, including invalid cameras. Kept separately
        # from the full prefix pad mask so text can never become a CVGE query.
        visual_mask = torch.cat(pad_masks, dim=1)
        ...
        return embs, pad_masks, att_masks, visual_mask
