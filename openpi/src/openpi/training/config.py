...
class ModelTransformFactory(GroupFactory):
    ...
    def __call__(self, model_config: _model.BaseModelConfig) -> _transforms.Group:
        geometry_inputs = (
            [_transforms.PreserveGeometryImages()]
            if isinstance(model_config, pi0_config.Pi0Config) and model_config.geometry.enabled
            else []
        )
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi0_cvge_libero",
        model=pi0_config.Pi0Config(
            geometry=GeometryConfig(enabled=True, train_policy="adapter_action"),
        ...
        # Supply --pytorch-weight-path and --model.geometry.vggt-weights-path.
        # VGGT source defaults to the sibling VGGDrive checkout.
        batch_size=8,
        num_train_steps=30_000,
    ),
    TrainConfig(
        name="pi0_cvge_omega_libero",
        model=pi0_config.Pi0Config(
            geometry=GeometryConfig(enabled=True, backbone="vggt_omega", train_policy="adapter_action"),
            pytorch_compile_mode=None,
        ),
        data=LeRobotLiberoDataConfig(
            repo_id="physical-intelligence/libero",
        ...
        # image_size defaults to 416 for the VGGT-Omega-1B-416 reproduction weights; pass
        # --model.geometry.image-size 512 (or 256) for the other released checkpoints.
        # Supply --pytorch-weight-path and --model.geometry.vggt-weights-path; the VGGT-Omega
        # source defaults to a sibling vggt-omega/ checkout or the installed package.
        batch_size=8,
        num_train_steps=30_000,
    ),
    TrainConfig(
    ...
    TrainConfig(
        name="pi05_cvge_libero",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            # Match pi05_libero. Generic pi0.5 configs default to discrete state;
            # set True explicitly when the task should use state in the text prefix.
        ...
        optimizer=_optimizer.AdamW(clip_gradient_norm=1.0),
        # Initialize with pi0.5 PyTorch weights and the original VGGT weights.
        batch_size=8,
        num_train_steps=30_000,
    ),
    TrainConfig(
        name="pi05_cvge_omega_libero",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=False,
        ...
        optimizer=_optimizer.AdamW(clip_gradient_norm=1.0),
        # Omega defaults to 416; use 512 or 256 with the corresponding weights.
